...
router.post('/fetchtimetable', async (req, res) => {
  ...
  try {
    ...
    for (const coursecode of coursecodes) {
      let temp = await query(`select faculty from faculty_subject where section='${section}' and dept='${dept}'	and sem ='${sem}' and	academic_year='${academic_year}' and coursecode='${coursecode["coursecode"]}';`)
      let free = {}
      for (const hf1 of temp) {
        let facultytimetable = await query(`
        select tt.day, tt.slot, tt.coursecode, tt.section, tt.dept, tt.sem from (faculty_subject fs inner join timetable tt on fs.coursecode = tt.coursecode and fs.section = tt.section and fs.sem = tt.sem and fs.academic_year = tt.academic_year) where fs.academic_year = '${academic_year}' and fs.faculty = '${hf1["faculty"]}' and MOD(fs.sem, 2) = '${odd === 'odd' ? 1 : 0}'; `);
        ...
        for (const ft of facultytimetable) {
          const index2 = free_slots.indexOf(ft["day"].concat("-".concat(ft["slot"])));
          free_slots.splice(index2, 1);
        }
        free[hf1["faculty"]] = free_slots;
        console.log(facultytimetable);
      }
      faculties[coursecode["coursecode"]] = free
    }
...
router.post('/updatetimetable', async (req, res) => {
  try {
    ...
    let course_faculty = {}
    let result1 = await query(`select * from active_sem`);
    let { odd, academic_year } = result1[0]
