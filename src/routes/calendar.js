...
router.post('/fetchtimetable', async (req, res) => {
  ...
  try {
    ...
    let faculties = {};
    for (let cc of coursecodes) {
      let temp = await query(`select faculty from subjects_handled where coursecode like '${cc.coursecode}'`);
      let free = {}
      for (let t of temp) {

        let facultytimetable = await query(`
        select tt.day, tt.slot, tt.coursecode, tt.section, tt.dept, tt.sem from (faculty_subject fs inner join timetable tt on fs.coursecode = tt.coursecode and fs.section = tt.section and fs.sem = tt.sem and fs.academic_year = tt.academic_year) where fs.academic_year = '${academic_year}' and fs.faculty = '${t["faculty"]}' and MOD(fs.sem, 2) = '${odd === 'odd' ? 1 : 0}'; `);
        ...
        for (let ft of facultytimetable) {
          const index = free_slots.indexOf(ft["day"].concat("-".concat(ft["slot"])));
          free_slots.splice(index, 1);
        }
        free[t["faculty"]] = free_slots;
      }
      faculties[cc["coursecode"]] = free;
    }
...
async function updateCalendarTable(date, section, dept, sem, academic_year, timetable, emergency_holidays) {
    ...
    if (!emergency_holidays.includes(d) && daymap[moment(d, "YYYY-MM-DD").format('dddd')] != 'SUN' && daymap[moment(d, "YYYY-MM-DD").format('dddd')] != 'SAT') {
      for (let ttobj in timetable) {
        ...
        if (daymap[moment(d, "YYYY-MM-DD").format('dddd')] == timetable[ttobj]["day"] && section == timetable[ttobj]["section"] && dept == timetable[ttobj]["dept"] && sem == timetable[ttobj]["sem"] && academic_year == timetable[ttobj]["academic_year"]) {
          await query(`insert into calendar values('${moment(d).format("YYYY-MM-DD")}','${timetable[ttobj]["slot"]}','${timetable[ttobj]["coursecode"]}','${timetable[ttobj]["section"]}','${timetable[ttobj]["dept"]}','${timetable[ttobj]["sem"]}','${timetable[ttobj]["academic_year"]}');`)
        }
      }
  ...
  try {
    ...
    for (const ent of faculty_subject) {
      if (ent.faculty !== '-') {
        ...
        result = await query(`select count(*) as entry from subjects_handled where faculty like '${ent.faculty}' and coursecode like '${ent.coursecode}'`);
    ...
    let temp = []
    for (let tempObj of emergency_holidays) {
      temp.push(tempObj["date"])
    }
    let date = [start_date,end_date];
    await updateCalendarTable(date, section, dept, sem, academic_year, timetable, temp)
...
async function getCalendarEvents({ forDate }, username) {
  ...
  let events = [];
  let start_date = new Date(forDate)
