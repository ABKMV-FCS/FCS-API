      ...
      <v-flex xs12 sm11 md9 lg8 xl6>
        <v-card class="elevation-12">
          <v-toolbar color="primary" dark flat>
            <v-toolbar-title>Manage Leave Request</v-toolbar-title>
          </v-toolbar>
          <v-divider></v-divider>
            ...
            >
              <template v-slot:[`item.actions`]="{ item }">
                ...
                >
                  ...
                  <v-card>
                    <v-card-title>
                      <span class>Accept Leave Request</span>
                    </v-card-title>
                ...
                >
                  ...
                  <v-card>
                    <v-card-title>
                      <span class>Reject Leave Request</span>
                    </v-card-title>
...
export default {
  ...
  methods: {
    async getUsers() {
      try {
        this.users = (await this.$http.get("/api/odandleave/showleaverequest")).data.requests;
      } catch (err) {
		console.log(err);
	  }
    },
    async acceptSubmit() {
      this.loading = true;
      try {
        let res = await this.$http.post("/api/odandleave/acceptleaverequest", { faculty: this.updateSelect });
    ...
    async rejectSubmit() {
      this.loading = true;
      try {
        let res = await this.$http.post("/api/odandleave/rejectleaverequest", {
